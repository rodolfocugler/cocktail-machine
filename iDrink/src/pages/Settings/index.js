...
function Settings() {
    ...
    const loadMachines = async () => {
        const machineResponse = await cocktailMachineApi.get('/machines');
        setMachines(!!machineResponse.data ? machineResponse.data : []);
    }

    const loadPumps = async () => {
        const pumpResponse = await cocktailMachineApi.get('/pumps');
    ...
    return (<>
            <Wrapper>
                <SidebarMenu
                    selected={"Settings"}
                    onSelect={(value) => {
                        if (value !== "Settings" && value !== "") history.push(`/?strCategory=${value}`);
                    }}
                />
